...
func _on_shake_pressed():
	...
	var new_time = max(remaining - 5.0, 0.1)
...
func _on_fish_won(fish_weight, fish_name, current_rar, current_fish):
	...
	if Global.index_data.has(fish_name.to_lower()):
		Global.index_data[fish_name.to_lower()] += 1
	else:
		Global.index_data[fish_name.to_lower()] = 1
	...
	if fish_name.to_lower() == "wyatt the wizard":
		...
		if !Global.cool_setting:
			...
			DialogueManager.add_next_dialogue("i am wyatt the wizard")
	...
	SaveLoad.SaveFileData.index_data = Global.index_data
	SaveLoad._save()
